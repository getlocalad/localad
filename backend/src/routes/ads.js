...
// ── PATCH /api/ads/:id ────────────────────────────────────────────────────────
adsRouter.patch('/:id', requireAuth, async (req, res, next) => {
  try {
    const { title, imageUrl, targetUrl, altText, postalCodes, startsAt, endsAt, isActive } = req.body;
    ...
    const result = await pool.query(
      `UPDATE ads SET
         ...
         postal_codes = COALESCE($7, postal_codes),
       ...
       RETURNING id`,
      [req.params.id, req.user.id, title, imageUrl, targetUrl, altText,
       postalCodes || null, startsAt || null, endsAt || null,
